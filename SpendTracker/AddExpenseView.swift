    ...
    var body: some View {
        NavigationStack {
            VStack {
                ...
                Button {
                    showAlert = true
                    addExpense(data: ExpenseElement(name: name, date: date, nominal: Int(nominal)!), categoryIndex: choosenCategory)
                } label: {
                    ZStack {
                        RoundedRectangle(cornerRadius: 8)
        ...
        .alert(isPresented: $showAlert) {
            Alert(title: Text("Success"), message: Text("You have successfully \nadded spending"), dismissButton: Alert.Button.default(Text("Ok"), action: {
